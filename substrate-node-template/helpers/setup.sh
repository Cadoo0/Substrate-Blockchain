curl http://127.0.0.1:9933 -H "Content-Type:application/json;charset=utf-8" -d '{
  "jsonrpc":"2.0",
  "id":1,
  "method":"author_insertKey",
  "params": [
    "aura",
    "swift clown ride welcome cattle sunset cactus cinnamon talent april rose fever",
    "0xfa86a0aae4d19beb805b4875834ba23a89b2ec2e1afaf0da7a0b0d6320e2cd5b"
  ]
}'
curl http://127.0.0.1:9933 -H "Content-Type:application/json;charset=utf-8" -d '{
  "jsonrpc":"2.0",
  "id":1,
  "method":"author_insertKey",
  "params": [
    "gran",
    "swift clown ride welcome cattle sunset cactus cinnamon talent april rose fever",
    "0xa26aa99bd34d0cecdeeac72781343ea6aee4ce7f515810fed95024a1d0860392"
  ]
}'
curl http://127.0.0.1:9934 -H "Content-Type:application/json;charset=utf-8" -d '{
  "jsonrpc":"2.0",
  "id":1,
  "method":"author_insertKey",
  "params": [
    "aura",
    "cattle gift horror moment desert agent slice real gold arctic mistake business",
    "0xa61adb6469b236bc624ed982833e32c0d520fd337eba861b4b763c909c1a580b"
  ]
}'
curl http://127.0.0.1:9934 -H "Content-Type:application/json;charset=utf-8" -d '{
  "jsonrpc":"2.0",
  "id":1,
  "method":"author_insertKey",
  "params": [
    "gran",
    "cattle gift horror moment desert agent slice real gold arctic mistake business",
    "0xa651934cfc6929b708d7e103898a8f10ed93b7461d8a551bd633283d39e7fbd8"
  ]
}'
